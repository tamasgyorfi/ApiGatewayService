...
public class RetrieveFriendsCommand extends CommandBase {
    ...
    protected String run() throws Exception {
        ...
        Optional<String> result = new RequestRunner().runRequest(endpoint, GSON.toJson(user));
        if (result.isPresent()) {
    ...
    protected String getFallback() {
        return GSON.toJson(new UserServiceErrorResponse("Unable to retrieve user's friends.", "token"));
    }
}
